...
// External handle pool init function (will be defined in mapping layer)
extern int handle_pool_init(void* pool);
...
int io_init(void) {
    ...
    // Initialize handle pool (external dependency)
    // if (handle_pool_init(&g_handle_pool) != 0) return IO_ERROR;
