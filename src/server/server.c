#include "server.h"
#include "transport/stdio_transport/stdio_transport.h"
#include "transport/tcp_transport/tcp_transport.h"
#include "transport/udp_transport/udp_transport.h"
#include "transport/http_transport/http_transport.h"
#include "transport/ws_transport/ws_transport.h"
...
int mcp_server_init(mcp_server_t* server, const mcp_server_config_t* config) {
    ...
    if (mcp_server_validate_config(config) != 0) {
        return -1;
    }
    ...
    if (mcp_adapter_init(server->mcp_adapter) != MCP_ADAPTER_OK) {
        mcp_server_log(server, "ERROR", "Failed to initialize MCP adapter");
        return -1;
    }
    ...
    if (!server->transport_managers) {
        mcp_server_log(server, "ERROR", "Failed to allocate transport managers");
        return -1;
    }
    ...
    if (config->enable_stdio) {
        ...
        if (mcp_server_add_transport(server, stdio_transport, &stdio_config) == 0) {
            mcp_server_log(server, "INFO", "STDIO transport initialized");
        }
    }
    ...
    if (config->enable_tcp) {
        ...
        if (mcp_server_add_transport(server, tcp_transport, &tcp_config) == 0) {
            mcp_server_log(server, "INFO", "TCP transport initialized");
        }
    }
    ...
    if (config->enable_udp) {
        ...
        if (mcp_server_add_transport(server, udp_transport, &udp_config) == 0) {
            mcp_server_log(server, "INFO", "UDP transport initialized");
        }
    }
    ...
    if (config->enable_http) {
        ...
        if (mcp_server_add_transport(server, http_transport, &http_config) == 0) {
            mcp_server_log(server, "INFO", "HTTP transport initialized");
        }
    }
    ...
    if (config->enable_websocket) {
        ...
        if (mcp_server_add_transport(server, ws_transport, &ws_config) == 0) {
            mcp_server_log(server, "INFO", "WebSocket transport initialized");
        }
    }
    
    server->initialized = true;
    mcp_server_log(server, "INFO", "MCP Server initialized successfully");
...
int mcp_server_start(mcp_server_t* server) {
    ...
    mcp_server_log(server, "INFO", "Starting MCP Server...");
    ...
    for (size_t i = 0; i < server->transport_count; i++) {
        ...
        if (transport_manager_connect(manager) == TRANSPORT_MANAGER_OK) {
            ...
            mcp_server_log(server, "INFO", log_msg);
        } else {
            char log_msg[128];
            snprintf(log_msg, sizeof(log_msg), "Failed to connect transport %zu", i);
            mcp_server_log(server, "WARNING", log_msg);
        }
    }
    
    server->running = true;
    mcp_server_log(server, "INFO", "MCP Server started and listening for connections");
...
int mcp_server_stop(mcp_server_t* server) {
    ...
    mcp_server_log(server, "INFO", "Stopping MCP Server...");
    ...
    mcp_server_log(server, "INFO", "MCP Server stopped");
...
void mcp_server_shutdown(mcp_server_t* server) {
    ...
    mcp_server_log(server, "INFO", "Shutting down MCP Server...");
    ...
    mcp_server_log(server, "INFO", "MCP Server shutdown complete");
}
...
int mcp_server_process_request(mcp_server_t* server, const char* raw_request, char* response, size_t response_size) {
    ...
    if (mcp_adapter_parse_request(raw_request, &request) != MCP_ADAPTER_OK) {
        ...
        return mcp_adapter_format_error("unknown", -32700, "Parse error", response, response_size);
    }
